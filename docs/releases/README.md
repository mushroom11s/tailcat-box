...
Pushing a `v*` tag on [mushroom11s/tailcat-box](https://github.com/mushroom11s/tailcat-box) builds unsigned macOS disk images and Windows NSIS setup programs and attaches those files to a GitHub Release. Per-tag notes live in this directory. The public README only summarizes that. This file is the maintainer checklist.

The [Release workflow](../../.github/workflows/release.yml) uses standard GitHub-hosted runners. It does not use larger runners.

| Runner | Installer |
| --- | --- |
| `macos-latest` | Apple Silicon, `tailcat-box-macos-arm64-<tag>.dmg` |
| `macos-15-intel` | Intel Mac, `tailcat-box-macos-amd64-<tag>.dmg`. `macos-13` was retired in December 2025. |
| `windows-latest` | Windows amd64 NSIS setup, `tailcat-box-windows-amd64-<tag>.exe` |
| `windows-11-arm` | Windows ARM64 NSIS setup, `tailcat-box-windows-arm64-<tag>.exe` |
...
| [Release](../../.github/workflows/release.yml) | Push of a `v*` tag, or **Run workflow** | `wails build` for the four targets above, package a `.dmg` or NSIS `.exe`, and publish a GitHub Release when the tag is real |
...
3. The workflow builds the installers and attaches the `.dmg` and `.exe` files themselves to the GitHub Release. The body is `docs/releases/<tag>.md` when that file exists. If it is missing, this README is copied into a short fallback. Do not force-push tags.

The filename version is the git tag, including the leading `v` (`tailcat-box-macos-arm64-v0.4.0.dmg`). Each Windows matrix cell produces a single-arch NSIS setup via `wails build -nsis` after `choco install nsis`. Each macOS cell wraps `tailcat-box.app` plus an Applications symlink in a UDZO disk image with stock `hdiutil`. Installers are unsigned (no notarization, no Authenticode).

The in-app updater looks for the `.dmg` or `.exe` first and still accepts the older `.zip` names from releases that shipped archives.
